...
- `ansible/roles/kernel/tasks/tmpdir.yml` — keeps `/tmp` on disk: masks `tmp.mount` (Arch enables the upstream tmpfs default via `local-fs.target.wants/tmp.mount`), strips a `tmpfs /tmp` line from `/etc/fstab`, and deploys `/etc/tmpfiles.d/tmp.conf`. That file **replaces** `/usr/lib/tmpfiles.d/tmp.conf` wholesale, so the `/var/tmp` rule is restated in the template. `D!` clears `/tmp` at boot only. All three tasks apply on the **next boot** — the running tmpfs mount is never unmounted, because in-flight `/tmp` sockets and lockfiles would break.
