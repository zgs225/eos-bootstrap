...
- `ansible/playbook.yml` — single playbook, `hosts: localhost`, `become: true`. Role order is significant: `packages → mise → services → network → kernel → logging → power → fingerprint → user → display`.
...
- `ansible/roles/packages/defaults/main.yml` — package lists: `pacman_packages`, `packages_pacman_absent` (actively removed; e.g. pacman `go`, which mise owns), `thunar_packages`, `vm_pacman_packages` (VM-only, auto-detected), `aur_packages`. Deliberately in `defaults/`, not `vars/`: Ansible only auto-loads `vars/main.yml` from a role, not sibling files. AUR is installed via `paru` with `become_user: target_user` and `PARU=1`; requires `paru` to already be on PATH (installed by `bootstrap.sh` step 2, not by Ansible).
...
- `ansible/roles/kernel/tasks/tmpdir.yml` — keeps `/tmp` on disk: masks `tmp.mount` (Arch enables the upstream tmpfs default via `local-fs.target.wants/tmp.mount`), strips a `tmpfs /tmp` line from `/etc/fstab`, and deploys `/etc/tmpfiles.d/tmp.conf`. That file **replaces** `/usr/lib/tmpfiles.d/tmp.conf` wholesale, so the vendor's age-based rules are restated in the template (`D /tmp … 10d`, `D /var/tmp … 30d`) — dropping them silently would leave `/tmp` with boot-time clearing only. `D!` clears `/tmp` at boot (the `!` makes it `--boot`-only, so the daily `systemd-tmpfiles --clean` never empties a running `/tmp`). All three tasks apply on the **next boot** — the running tmpfs mount is never unmounted, because in-flight `/tmp` sockets and lockfiles would break.
- `ansible/roles/logging/` — journald retention drop-in (`SystemMaxUse`/`MaxRetentionSec`/`Compress` via `templates/journald.conf.j2`, one-shot `journalctl --vacuum-size` to shrink existing journals to the cap), global logrotate `compress`, generic logrotate rules for unmanaged text logs (`templates/logrotate-eos.conf.j2` → `/etc/logrotate.d/eos`: Xorg, xrdp, installer relics), one-shot gzip of pre-existing uncompressed rotated logs, and `logrotate.timer` enablement. Tune via `defaults/main.yml` (`logging_journald_system_max_use_mb` etc.).
...
- `ansible/roles/packages/tasks/thunderbolt.yml` — auto-detects a Thunderbolt/USB4 controller via `lspci` and deploys `/etc/udev/rules.d/90-thunderbolt.rules` (auto-authorizes **any** router on `add` — generic dock hotplug, PCIe-tunneled NICs appear plug-and-play) plus a `/etc/modules-load.d/thunderbolt.conf` fragment. **No pacman package**: the `thunderbolt` module ships with the kernel. Do **not** install `bolt`/boltd — it would take over authorization and fight the udev rule. Assumes `security=user` with IOMMU DMA protection (the Intel default); this is a blanket authorize-all, so the only DMA containment is VT-d.
...
- **Handler names must be unique across roles.** All roles run against localhost in a single play, so handlers share one namespace and `notify` matches by name globally. A `notify: Reload udev` in the packages role silently fires the *fingerprint* role's identically-named handler (last-defined wins), leaving your own handler as dead code — it only "worked" because both happened to run `udevadm control --reload-rules`. The thunderbolt handlers are therefore named `... for thunderbolt` to stay self-contained. Also: handlers execute in **definition** order, not notify order — the thunderbolt udev-reload handler is defined *before* its trigger handler so the trigger fires against freshly reloaded rules (get this backwards and the trigger runs against stale rules and authorizes nothing).
...
- **libfprint upgrades wipe the fingerprint driver.** The FocalTech proprietary driver is deployed at `/usr/lib/libfprint-2.so.2.0.0`, a path owned by the `libfprint` package — every pacman upgrade of libfprint restores the stock library, losing the patchelf shim injection, and fprintd reports "No devices available". The pacman hook `/etc/pacman.d/hooks/focaltech-libfprint-restore.hook` (→ `/usr/local/sbin/focaltech-restore-driver`, both deployed by the fingerprint role) re-injects the driver PostTransaction. Manual fix if it ever regresses: run the restore script or `ansible-playbook ... --tags fingerprint`. Symptom check: `fprintd-list $USER` + `patchelf --print-needed /usr/lib/libfprint-2.so.2.0.0 | grep shim`.
- **polkit fingerprint PAM is a full drop-in, not lineinfile.** `/etc/pam.d/polkit-1` doesn't exist by default (factory config is `/usr/lib/pam.d/polkit-1`), so the fingerprint role deploys the complete file (pam_fprintd `sufficient` + system-auth fallthrough, same pattern as sudo). This is what makes fingerprint work in all polkit prompts — including Bitwarden's "Unlock with system authentication" (polkit action `com.bitwarden.Bitwarden.unlock`, policy auto-installed by Bitwarden on first enable). Bitwarden additionally requires a **polkit authentication agent** in the session (i3 has none by default): `polkit-gnome` is in `pacman_packages`, the autostart `exec` line lives in the dotfiles i3 config. Without the agent, enabling the feature in Bitwarden silently does nothing.
- **The `lizardbyte` pacman repo DB is fetched from github.com.** When github is unreachable, `pacman --sync --refresh` (run by the packages role's pacman task) fails on that one repo and aborts the whole play before later roles run. Workaround for urgent single packages: `pacman -S --needed <pkg>` (no refresh; the Arch mirrors' DBs usually sync fine), then re-run the playbook once github is reachable again.
